...
const SignUp = () => {
  ...
  return (
    <Container maxWidth="xs" style={{ textAlign: "center", background: "#fff", padding: "30px", borderRadius: "10px", boxShadow: "0px 0px 10px rgba(0,0,0,0.1)" }}>
      ...
      {role === "Admin" && (
        <>
          <TextField fullWidth margin="normal" label="O‘quv Markazi" variant="outlined" value={adminCode} onChange={(e) => setAdminCode(e.target.value)} />
          <TextField fullWidth margin="normal" type="password" label="manzil" variant="outlined" value={adminKey} onChange={(e) => setAdminKey(e.target.value)} />
        </>
      )}

      <Button fullWidth variant="contained" color="primary" style={{ marginTop: "15px" }} onClick={handleSignUp}>
        Sign Up
      </Button>
